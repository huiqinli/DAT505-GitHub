...
```
This code creates a scene, a camera, lights, four objects, and it adds the geometries to the scene. They are designed in certain positions to express an aesthetic concept of  movement balance.Both 0f them are rotating. It then creates a renderer for the scene and camera, and it adds that viewport to the document.body element. Finally, it renders the geometries within the scene for the camera.
...
`create geometries with differnt materials`
...
`set the initial speed and continuously rotate the mesh`
