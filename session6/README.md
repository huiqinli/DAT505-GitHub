...
`the world consists of a plane and buildings`
...
function setupWorld() {
  ...
  floor.rotation.x = -1 * Math.PI;
  floor.receiveShadow = true;
  scene.add(floor);

  //Settings for models and material
  var geometry = new THREE.SphereGeometry( 0.5, 0.5, 3 );
  //geometry.applyMatrix( new THREE.Matrix4().makeTranslation( 0, 0.5, 0 ) );
  var material = new THREE.MeshPhongMaterial({overdraw: true, color: 0x00FFFF, wireframe: true});
  ...
  for (var i = 0; i < 300; i++) {
    ...
    building.position.y = Math.floor( Math.random() * 250 - 100 ) * 4;
